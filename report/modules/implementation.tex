...
Furtheremore each neighbour has it's own {\it ack\_timer\_id} that way we ensure that there's only one instance per neighbour
...
We have further implemented {\it packetTimerMessage} a struct that is sent to the library whenever we start a timer.
...
Our implementation of {\it start\_timer} is as follows. We first allocate memory for our {packetTimerMessage}
The sequence number is updated accordingly to our parameter, on the {\it start\_ack\_timer} function however, we set the sequence number to -1 (not used) since it redundant in that case.
...
\\~
To stop the timers we simply invoke the subnet function: {\it StopTimer} with the id of the timer
